...
export async function POST(req: NextRequest) {
  const rate = checkApiRateLimit(req, "decibel-withdraw-build", 60, 60_000);
  if (!rate.allowed) {
  ...
  try {
    ...
    console.error("[decibel-withdraw] build failed:", message);
    return NextResponse.json(
      { error: "Could not prepare the Decibel withdrawal transaction" },
