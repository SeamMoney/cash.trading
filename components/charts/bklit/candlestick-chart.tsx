...
export interface CandlestickChartProps {
  ...
  xDomain?: [Date, Date];
  /** Number of time slots represented by the domain. Keeps sparse/gapped candles from becoming artificially wide. */
  xDomainSlotCount?: number;
  /** Never merge OHLC buckets across a real outage while downsampling. */
  maxDataGapMs?: number;
  /** Optional explicit price domain. Useful when overlays extend beyond candle highs/lows. */
  yDomain?: [number, number];
  /** Fractional vertical domain padding. Default: 0.05. */
  yPaddingRatio?: number;
  /** Native touch behavior. Defaults to vertical page scrolling with horizontal chart gestures. */
  touchAction?: React.CSSProperties["touchAction"];
  /** Enable bklit's internal range-selection gesture. Default: true. */
  selectionEnabled?: boolean;
  /** Maximum pointer distance from a real candle before inspection clears. */
...
const ChartCore = memo(function ChartCore({
  ...
  const slotCount =
    xDomainSlotCount != null && Number.isFinite(xDomainSlotCount)
      ? Math.max(1, xDomainSlotCount)
      : data.length;
  const slotWidth = innerWidth / Math.max(slotCount, 1);
  const xScale = useMemo(() => {
    let minTime = xDomain ? xDomain[0].getTime() : Number.POSITIVE_INFINITY;
    let maxTime = xDomain ? xDomain[1].getTime() : Number.NEGATIVE_INFINITY;
    if (!xDomain) {
  ...
  const yScale = useMemo(() => {
    ...
    if (yDomain && yDomain.every(Number.isFinite)) {
      minVal = Math.min(yDomain[0], yDomain[1]);
      maxVal = Math.max(yDomain[0], yDomain[1]);
    }
    const safePaddingRatio = Number.isFinite(yPaddingRatio)
      ? Math.min(0.2, Math.max(0.005, yPaddingRatio))
      : 0.05;
    const padding = (maxVal - minVal) * safePaddingRatio || 1;
    return scaleLinear({
      range: [innerHeight, 0],
      domain: [minVal - padding, maxVal + padding],
      nice: safePaddingRatio >= 0.03,
    });
  }, [innerHeight, data, yDomain, yPaddingRatio]);
  ...
  const renderData = useMemo(
    () => decimateOhlcData(data, maxRenderPointsForWidth(innerWidth), maxDataGapMs),
    [data, innerWidth, maxDataGapMs]
  );
  ...
  const contextValue = useMemo(
    () => ({
      ...
      margin,
      columnWidth,
      candlestickPositiveFill: "var(--chart-candle-up)",
      candlestickNegativeFill: "var(--chart-candle-down-stroke)",
      tooltipData,
      setTooltipData,
      containerRef,
      lines,
      ...
      <ParentSize debounceTime={10}>
        {({ width, height }) => (
          <ChartInner
            ...
            maxDataGapMs={maxDataGapMs}
            maxTooltipDistanceMs={maxTooltipDistanceMs}
            yDomain={yDomain}
