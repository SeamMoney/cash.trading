...
function VaultPositionsSkeleton() {
  return (
    <div aria-label="Loading vault positions">
      {Array.from({ length: 8 }, (_, index) => (
        <div
          key={index}
          className="grid grid-cols-[1.35fr_0.8fr_1fr] items-center gap-2 border-b border-white/[0.04] px-3 py-3"
        >
          <div className="h-3 w-24 animate-pulse rounded-[var(--radius-xs)] bg-white/[0.06]" />
          <div className="ml-auto h-3 w-14 animate-pulse rounded-[var(--radius-xs)] bg-white/[0.05]" />
          <div className="ml-auto h-3 w-16 animate-pulse rounded-[var(--radius-xs)] bg-white/[0.05]" />
        </div>
      ))}
    </div>
  );
    ...
    <section className="relative h-[460px] overflow-hidden">
      <header className="flex h-[44px] items-center justify-between border-y border-white/[0.06] px-3">
        <div className="min-w-0">
          <div className="truncate font-sans text-xs font-semibold text-zinc-200">
            Open positions
          </div>
        ...
        {updatedAt != null && (
          <span aria-live="polite" className="shrink-0 text-[11px] tabular-nums text-zinc-600">
      ...
      {status === "error" ? (
        <div className={cn(TABLE_HEIGHT_CLASS, "flex flex-col items-center justify-center gap-3 px-6 text-center")}>
          <div>
            <div className="text-xs font-semibold text-zinc-300">Positions unavailable</div>
            <p className="mt-1 text-pretty text-[11px] leading-4 text-zinc-600">
          ...
          <button
            ...
            className="rounded-[var(--radius-sm)] border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-[11px] font-semibold text-zinc-300 transition-[transform,background-color] duration-150 hover:bg-white/[0.08] active:scale-[0.97]"
          >
            Retry
          </button>
        </div>
        ...
        <div className={cn(TABLE_HEIGHT_CLASS, "flex flex-col items-center justify-center gap-3 px-6 text-center")}>
          <div>
            <div className="text-xs font-semibold text-zinc-300">No open positions</div>
            <p className="mt-1 text-pretty text-[11px] leading-4 text-zinc-600">
        ...
        <>
          ...
          >
            <table className="w-full table-fixed text-left text-[11px] tabular-nums">
              <thead className="sticky top-0 z-10 bg-background-tertiary text-[11px] font-medium uppercase text-zinc-600">
                <tr>
                    ...
                    >
                      <td className="min-w-0 px-3 py-2.5 align-top">
                        <div className="flex min-w-0 items-center gap-1.5">
                          ...
                          <span
                            className={cn(
                              "shrink-0 rounded-[var(--radius-xs)] px-1 py-0.5 text-[11px] font-bold",
                              position.isLong
          ...
          {canScrollUp && (
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-x-0 top-[44px] z-20 flex h-5 items-center justify-center border-b border-white/[0.04] bg-background-secondary/95 text-[11px] text-zinc-600"
            >
              ↑
            </div>
          )}
          {canScrollDown && (
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex h-7 items-center justify-center border-t border-white/[0.05] bg-background-secondary/95 text-[11px] font-semibold uppercase text-zinc-600"
            >
              More ↓
            </div>
          )}
